...
import { getNormalizedDaysOfMonth } from "./month.util";
...
function ensureDateRange(range?: DateRange): DateString[] {
	...
	const start = ensureDateString(range.start, undefined);
	const end = ensureDateString(range.end, undefined);
...
function MonthRange(props: MonthRangeProps) {
	...
	// props validations
	const year = bound(
		notNullOrUndef(props.year, now.getFullYear()) | 0,
		0,
		9999,
	);
	const month = bound(notNullOrUndef(props.month, now.getMonth()) | 0, 0, 11);
	...
	const onClick = useCallback(
		(event: React.MouseEvent<HTMLButtonElement>) => {
			const dateString = ensureDateString(
			...
			if (currentValue.length === 1) {
				const newState = currentValue[0] < dateString
		...
		<div className={classname(style.wrapper, style.range, className)}>
			...
			<div className={style.days}>
				{days.map((day) => {
					...
					const isAnotherMonth = day.month !== month ||
						day.year !== year;
					const isDisabled = isAnotherMonth ||
						disabled ||
					...
					const className = classname([
						now.toISOString().startsWith(dateString) && style.today,
						...
						!isRangeSelected && dateString === hoveredDate &&
						style.hover,
					]);
					...
					return (
						<BaseButton
							...
							onMouseOver={isMouseEventDisabled
								? undefined
								: onMouseOver}
							onMouseOut={isMouseEventDisabled
								? undefined
								: onMouseOut}
						>
							{day.date}
...
export { ensureDateRange, MonthRange };
